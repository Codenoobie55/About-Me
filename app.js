...
var vacationSpots = ['Paris', 'New York', 'Barcelona','England'];

  for (var i = 0; i < vacationSpots.length; i++){
 console.log(vacationSpots[i]);
}
  var howManyCountries = 4;
  for(var i = 0; i < 1; i++){
    var quizQuestion = prompt('What countries did James visit ?');
    if(quizQuestion == vacationSpots.length) {
      alert('too high');
    } else if(quizQuestion < howManyFish){
      alert(' too low');
    }else{
